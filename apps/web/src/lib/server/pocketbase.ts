...
import { AUTH_COOKIE_NAME, createPocketBase, POCKETBASE_URL } from '#/lib/pocketbase'
...
/**
 * A fresh, request-scoped PocketBase client hydrated from the
 * `AUTH_COOKIE_NAME` cookie. Never reuse a single instance across requests
 * here: on Cloudflare Workers, module-scope state can be shared across
 * concurrent requests in the same isolate, and an authenticated client
 * shared that way would leak one user's session into another user's
 * request.
 */
