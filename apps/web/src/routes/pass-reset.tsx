...
export const Route = createFileRoute('/pass-reset')({
...
function PassResetPage() {
  ...
  const { requestPassReset } = useAuthActions()
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-ground px-4">
      <div className="w-full max-w-sm rounded-lg border border-border bg-surface p-8 shadow-sm">
        ...
        <form onSubmit={handleRequestPass} className="flex flex-col gap-4">
          ...
          >
            {submitting ? 'Requesting…' : 'Reset your password'}
          </button>
        </form>
      </div>
    </div>
